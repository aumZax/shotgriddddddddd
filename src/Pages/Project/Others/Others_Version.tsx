...
import {
    ...
    ExternalLink, Eye, FileText, Filter, HardDrive,
    Image, Search, SortAsc, SortDesc, Trash2, Upload,
...
export default function Others_Version() {
    ...
    // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    const handleNavigateToEntity = async (entityType: string, entityId: number, entityName: string) => {
        if (entityType === "shot") {
            try {
                const projectId = JSON.parse(localStorage.getItem("projectId") || "null");
                ...
                for (const group of res.data) {
                    const shot = group.shots?.find((s: any) => s.id === entityId);
                    if (shot) {
                        foundShot = { ...shot, sequence: group.category, assets: shot.assets || [] };
                        break;
                    }
                }
                if (foundShot) {
            ...
            try {
                ...
                let foundAsset = null;
                for (const group of res.data) {
                    const asset = group.assets?.find((a: any) => a.id === entityId);
                    if (asset) { foundAsset = { ...asset, category: group.category }; break; }
                }
                if (foundAsset) {
            ...
            try {
                ...
                const sequence = res.data.find((seq: any) => seq.id === entityId);
                if (sequence) {
